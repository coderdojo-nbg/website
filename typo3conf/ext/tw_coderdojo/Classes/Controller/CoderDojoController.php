...
class CoderDojoController extends \TYPO3\CMS\Extbase\Mvc\Controller\ActionController
{
    ...
    public function downloadAction($date)
    {
        ...
        $this->downloadRecipients($recipients, $date->getDojoNumber().'-');
    }

    /**
     * Download all attendees CSV list
     *
     * @return void
     */
    public function downloadAllAction()
    {
        $ninjas = new ObjectStorage();
        $helpers = new ObjectStorage();
        $mentors = new ObjectStorage();

        // Run through all Dojos
        /** @var Date $date */
        foreach ($this->dateRepository->findAll() as $date) {
            $ninjas->addAll($date->getNinjas());
            $helpers->addAll($date->getHelpers());
            $mentors->addAll($date->getMentors());
        }

        $recipients = [];
        $this->createRecipientList($recipients, self::$NINJA, $ninjas);
        $this->createRecipientList($recipients, self::$HELPER, $helpers);
        $this->createRecipientList($recipients, self::$MENTOR, $mentors);

        $this->downloadRecipients($recipients);
    }
     ...
     */
    protected function downloadRecipients(array $recipients, $filename = '')
    {
        if (count($recipients)) {
            $tmpFileName = 'CoderDojo-'.$filename.date('Ymd').'.csv';
